...
class RewardThresholdCurriculum(Curriculum):
    ...
    def update(self, bin_inds, task_rewards, success_thresholds, local_range=0.5, isBase=True):
        ...
        if isBase:
            ...
            # Add forgetting: decay all weights slightly
            self.weights = np.clip(self.weights * 0.99, 0.01, 1.0)  # 1% decay per update

            self.weights[bin_inds[is_success]] = np.clip(self.weights[bin_inds[is_success]] + 0.1, 0, 1)
            adjacents = self.get_local_bins(bin_inds[is_success], ranges=local_range)
            for adjacent in adjacents:
                adjacent_inds = np.array(adjacent.nonzero()[0])
                # For adjacent bins, use smaller increment
                self.weights[adjacent_inds] = np.clip(self.weights[adjacent_inds] + 0.05, 0, 1) # Smaller than direct success

            if len(is_success) > 0:
                success_rate = is_success.mean()
                print(f"[Curriculum] Success rate: {success_rate:.2f}, "
                    f"Weight distribution: mean={self.weights.mean():.3f}, "
                    f"max={self.weights.max():.3f}, min={self.weights.min():.3f}")
