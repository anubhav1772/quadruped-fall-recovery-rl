import torch
from aliengo_gym_deploy.utils.generate_path import VelocityProfile      
...
class RCControllerProfile(CommandProfile):
    ...
    def get_command(self, t, probe=False):
        ...
        # GENERATE PATH
        # Monotonic ramp (no reversal)
        [x_vel_cmd, y_vel_cmd, yaw_vel_cmd] = self.path.generate_linear_ramp(v_x_max=1.2)
        # [x_vel_cmd, y_vel_cmd, yaw_vel_cmd] = self.path.generate_symmetric_ramp(v_x=0.8)
        ...
        x_vel_cmd, y_vel_cmd, yaw_vel_cmd = x_vel_cmd, 0.0, 0.0
        body_height_cmd = 0.0
