...
class GaitPolicyWrapper(gym.Wrapper):
    ...
    def reset(self):
        """
        Reset inner env (HistoryWrapper) and build initial observation
        """

        # Reset underlying env (HistoryWrapper)
        # Returns obs = {"obs", "privileged_obs", "obs_history"} for pi2
        _ = self.env.reset()

        # Zero/reset Policy 1 state
        self.gaits.zero_()
        # self.gait_indices[:] = 0.0
        self.pi1_obs_history.zero_()  # history for policy 1

        self.current_vel_cmds[:, 0] = torch.empty(self.num_envs).uniform_(0.3, 1.0)
        self.current_vel_cmds[:, 1] = 0.0
        self.current_vel_cmds[:, 2] = 0.0

        # Build obs for policy 1 (fills self.pi1_obss, self.privileged_pi1_obss, self.pi1_obs_history)
        # Build single-step obs for Policy 1
    ...
    def step(self, gaits):
        ...
        """
        # if self.pi1_counter % self.pi1_decimation == 0:
        #     # update pi1 command
        #     self.current_pi1_gaits[:] = gaits
        ...
        self.wtw.set_env_commands(self.current_vel_cmds, self.gaits)
        ...
        for i in range(self.win_len):
            ...
            rew_accum += 50.0 * rew_pi2 * alive / self.win_len
        ...
        if dead_ids.numel() > 0:
            self.env.reset_idx(dead_ids)
            self.env.compute_observations()

        valid_counts = torch.clamp(valid_counts, min=1.0)

        # Means only for logging / obs
        lin_vel_mean = self.lin_vel_buf.sum(dim=1) / valid_counts.unsqueeze(1)
        ang_vel_mean = self.ang_vel_buf.sum(dim=1) / valid_counts.unsqueeze(1)
        #rew_ll_mean = rew_accum / valid_counts
        rew_ll_mean = rew_accum
        ...
        distance = torch.clamp(distance, min=1e-3)
        ...
        reward = rew_ll_mean + rew_energy + 0.2 * rew_smooth
        ...
        self.episode_sums["rew_ll"] += rew_ll_mean * mask
        ...
        if alive.any():
            # Step-wise metrics
            info["train/step"]["rew_energy"] = rew_energy[alive].mean().item()
            info["train/step"]["rew_smooth"] = rew_smooth[alive].mean().item()
            info["train/step"]["rew_ll_mean"] = rew_ll_mean[alive].mean().item()
            info['train/step']["rew_ll_min"] = rew_ll_mean[alive].min().item()
            info['train/step']["rew_ll_max"] = rew_ll_mean[alive].max().item()
            # info["train/step"]["rew_tracking_lin"] = rew_lin[alive].mean().item()
            # info["train/step"]["rew_tracking_ang"] = rew_ang[alive].mean().item()
            info["train/step"]["rew_total"] = reward[alive].mean().item()

            info["train/step"]["vx_cmd_abs"] = vx_cmd[alive].abs().mean().item()
            info["train/step"]["vx_abs"] = vx[alive].abs().mean().item()
            # info["train/step"]["vx_err"] = ((vx_cmd[alive] - vx[alive]) ** 2).mean().item()

            # info["train/step"]["yaw_cmd_abs"] = yaw_cmd[alive].abs().mean().item()
            # info["train/step"]["yaw_abs"] = yaw[alive].abs().mean().item()
            # info["train/step"]["yaw_err"] = ((yaw_cmd[alive] - yaw[alive]) ** 2).mean().item()
            #info["train/step"]["energy_per_meter"] = (energy[alive] / distance[alive]).mean().item()

            #info["train/step"]["energy"] = energy[alive].mean().item()
            # info["train/step"]["distance"] = distance[alive].mean().item()

            info["train/step"]["power_total"] = mean_power[alive].mean().item()
            #info["train/step"]["CoT"] = self.compute_CoT()[alive].mean().item()
            info["train/step"]["CoT"] = cot[alive].mean().item()
        else:
            info["train/step"]["rew_energy"] = rew_energy.mean().item()
            info["train/step"]["rew_smooth"] = rew_smooth.mean().item()
            info["train/step"]["rew_ll_mean"] = rew_ll_mean.mean().item()
            info['train/step']["rew_ll_min"] = rew_ll_mean.min().item()
            info['train/step']["rew_ll_max"] = rew_ll_mean.max().item()
            # info["train/step"]["rew_tracking_lin"] = rew_lin.mean().item()
            # info["train/step"]["rew_tracking_ang"] = rew_ang.mean().item()
            info["train/step"]["rew_total"] = reward.mean().item()

            info["train/step"]["vx_cmd_abs"] = vx_cmd.abs().mean().item()
            info["train/step"]["vx_abs"] = vx.abs().mean().item()
            # info["train/step"]["vx_err"] = ((vx_cmd - vx) ** 2).mean().item()

            # info["train/step"]["yaw_cmd_abs"] = yaw_cmd.abs().mean().item()
            # info["train/step"]["yaw_abs"] = yaw.abs().mean().item()
            # info["train/step"]["yaw_err"] = ((yaw_cmd - yaw) ** 2).mean().item()
            #info["train/step"]["energy"] = energy.mean().item()
            # info["train/step"]["distance"] = distance.mean().item()
            info["train/step"]["power_total"] = mean_power.mean().item()
            #info["train/step"]["CoT"] = self.compute_CoT().mean().item()
            info["train/step"]["CoT"] = cot.mean().item()
