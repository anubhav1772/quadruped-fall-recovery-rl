...
class GaitPolicyWrapper(gym.Wrapper):
    ...
    def step(self, gaits):
        ...
        self.env.commands[alive_ids, 3:3 + self.gaits.shape[1]] = self.gaits[alive_ids]
        ...
        for i in range(self.win_len):
            ...
            self.lin_vel_buf[self.alive, i] = lin_vel[self.alive]
            self.ang_vel_buf[self.alive, i] = ang_vel[self.alive]

            # LL reward (window-normalized)
            rew_accum += 25.0 * rew_pi2 * self.alive.float() / self.win_len

            # power
            self.power_buf[:, i] = torch.sum(torch.abs(self.env.torques * self.env.dof_vel), dim=1) * self.alive.float()
            ...
            #self.power_buf[self.alive, i] = power[self.alive]

            # distance
            #step_dist = torch.norm(lin_vel[:, :2], dim=1) * self.dt
            #distance[self.alive] += step_dist[self.alive]
        ...
        self.env.defer_command_resample = False
        ...
        #reward = (rew_ll_win + rew_energy + 0.2 * rew_smooth) * mask
        #rew_log = torch.log(reward)
        #reward = 3 * (rew_log - rew_log.mean().detach())
        #reward = torch.clamp(reward, -3, 3)

        rew_total = rew_ll_win + rew_energy + 0.2 * rew_smooth
        ...
        self.episode_sums["rew_ll"] += rew_ll_win * mask
