...
#     pi1_dir = "/home/anubhav1772/Documents/TEST/WTW-Aliengo/runs/gait-conditioned-agility/2026-01-29/train_gait_generator/222020.849949"
#     pi2_dir = "/home/anubhav1772/Documents/TEST/WTW-Aliengo/runs/gait-conditioned-agility/2025-10-21/train/022109.278720"
...
def load_and_run_policy(label, experiment_name, args, max_vel=1.0, max_yaw_vel=1.0):
    ...
    # Load config (LL config only)
    with open(pi2_dir + "/parameters.pkl", "rb") as file:
        pkl_cfg = pkl.load(file)
        cfg = pkl_cfg["Cfg"]

    # State estimator + commands
    se = StateEstimator(lc)
    ...
    if args.deploy_policy == "sim":
        pi2 = load_policy_sim(pi2_dir)
    elif args.deploy_policy == "offline":
        pi2 = load_policy_offline(pi2_dir)
    elif args.deploy_policy == "online":
        pi2 = load_policy_online(pi2_dir)
    else:
        raise ValueError("Unknown deploy_policy")

    # Build hierarchical policy
    ll_freq = 1.0 / control_dt      # 50 Hz
    hl_freq = 2.0                   # MUST match training
    hl_decimation = int(ll_freq / hl_freq)  # 25
